...
- CommonJS 모듈과 ES 모듈의 차이점
  ...
  | \_\_filename,<br/>\_\_dirname | 사용 가능     | 사용 불가능(\_\_filename 대신 `import.meta.url` 사용) |
...
2. console<sup>\*</sup>
   ...
   - console.log(): 평범한 로그
   - console.table(): 테이블로 표시할 로그
   - console.error(): 에러 로깅
   - console.dir(): 객체 로깅
   - `console.trace()`: 호출스택 로깅(함수 내부에서 사용)
3. process<sup>\*</sup>
   - 현재 실행중인 노드 프로세스에 대한 정보를 담고있음
   - `process.cwd()`: 실행중인 노드 프로세스 경로
   - `process.env`: 시스템 환경 변수
     - 노드에 직접 영향을 미치는 변수도 있음
       ```bash
       NODE_OPTIONS=--max-old-space-size=8192 # 노드 옵션: 메모리 사이즈
       UV_THREADPOOL_SIZE=8 # 스레드 갯수
       ```
   - `process.nextTick(콜백)`: 이벤트 루프가 다른 콜백 함수보다 우선적으로 처리함
   - `process.exit(코드)`: 코드가 없거나 0이면 정상 종료(그 외는 비정상 종료)
4. 기타 내장 객체
   - URL, URLSearchParams
...
<details>
<summary><sup>*</sup>console example</summary>
<div markdown=1>
...
<details>
<summary><sup>*</sup>process example</summary>
<div markdown=1>
...
- <details>
  ...
  • os.arch(): process.arch와 동일합니다.<br/>
  • os.platform(): process.platform과 동일합니다.<br/>
  • os.type(): 운영체제의 종류를 보여줍니다.<br/>
  • os.uptime(): 운영체제 부팅 이후 흐른 시간(초)을 보여줍니다.<br/>
  • os.hostname(): 컴퓨터의 이름을 보여줍니다.<br/>
  • os.release(): 운영체제의 버전을 보여줍니다.<br/>
  • os.homedir(): 홈 디렉터리 경로를 보여줍니다.<br/>
  • os.tmpdir(): 임시 파일 저장 경로를 보여줍니다.<br/>
  • <strong>os.cpus()</strong>: 컴퓨터의 코어 정보를 보여줍니다.<br/>
  • os.freemem(): 사용 가능한 메모리(RAM)를 보여줍니다.<br/>
  • os.totalmem(): 전체 메모리 용량을 보여줍니다.<br/>
...
- 폴더와 파일의 경로를 쉽게 조작하도록 도와주는 모듈
  ...
  | --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
  |.sep | 경로의 구분자<br/>_\*WINDOW는 "\\", POSIX(LINUX & MAC)는 "/"_ |
  |.delimiter | 환경 변수의 구분자 (process.env.PATH를 입력하면 여러 개의 경로가 이 구분자로 구분되어 있음)<br/>_\*WINDOW는 ";", POSIX는 ":"_ |
  |.dirname(경로) | 파일이 위치한 폴더 경로 |
  |.extname(경로) | 파일의 확장자 |
  |.basename(경로, [확장자]) | 파일의 이름<br/>_\*두 번째 인수를 넣을 경우 파일의 이름만 표시_ |
  |.parse(경로) | 파일 경로를 root, dir, base, ext, name으로 구분한 객체 |
  |.format(객체) |path.parse()의 반환값을 합친 경로 |
  |.normalize(경로) | "/"나 "\\"를 잘못 사용했을 때 올바르게 변환한 경로 |
  |.isAbsolute(경로) | 절대경로인지 여부 |
  |.relative(기준경로, 목표경로) | 기준경로에서 목표경로로 가는 경로 |
...
- `.searchParams`
  |Properties & Methods | Return or Details|
  |---|---|
  |.getAll(키)|키에 해당하는 모든 값|
  |.get(키)|키에 해당하는 첫 번째 값|
  |.has(키)|해당 키의 존재 여부|
  |.keys()|모든 키를 포함한 iterator(반복자 객체)|
  |.values()|모든 값를 포함한 iterator(반복자 객체)|
  |.append(키, 값)|해당 키를 추가, 같은 키의 값이 있다면 유지하면서 하나 더 추가|
  |.set(키, 값)|해당 키를 제거|
...
2. 양방향 암호 - 암호화된 문자열을 복호화할 수 있음

   - 대칭형 암호 - 같은 키
   - 비대칭 암호 - 다른 키 (e.g. https)
   ...
   \*키는 AWS kms등으로 관리해 줘야 함

#### 6. utils

- 각종 편의 기능을 모아둔 모듈
- .deprecate(콜백, 메시지): 함수가 deprecated 처리되었음을 알림
- .promisify(콜백): 콜백 패턴을 프로미스 패턴으로 변경(but 콜백이 (err, data) => {} 형식이어야 함)
- <details>
  <summary>example</summary>
  <div markdown=1>
...
- https://www.npmjs.com/package/crypto-js (SHA512(단반향), AES(대칭), RSA(비대칭))
